...
import { currentNetwork, erc20s, networks, zeroAddress } from "@defi.org/web3-candies";
...
describe.only("Paraswap", () => {
  [
    { name: "Ethereum USDC", chainId: networks.eth.id, usdc: erc20s.eth.USDC },
    { name: "Fantom USDC", chainId: networks.ftm.id, usdc: erc20s.ftm.USDC },
    { name: "Polygon USDC", chainId: networks.poly.id, usdc: erc20s.poly.USDC },
  ].map((c) => {
    describe(c.name, () => {
      ...
      it("gas prices", async () => {
        ...
        expect(result.low).bignumber.lte(result.medium).lte(result.high).lte(result.instant);
      });
    });
  });
});
